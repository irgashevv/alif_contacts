...
class Contacts
{
    ...
    public function deleteByPhoneId($id, $contactId)
    {
        $this->mysqli->query("DELETE FROM {$this->user_phone_table} where id = $id");
        header("Location: /?model=contacts&action=update&id=" . $contactId);
    }

    public function deleteByEmailId($id, $contactId)
    {
        $this->mysqli->query("DELETE FROM {$this->user_email_table} where id = $id");
        header("Location: /?model=contacts&action=update&id=" . $contactId);
    }
